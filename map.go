...
var item string = color(30, 230, 30) + "?"
var chars = []string{floor, gold, floor, floor, floor, wall, wall1, wall2, wall3}

func (_this *Map) draw(p Player, vecs []Item) {
	pos(0, 0)
	scale := 2.
	for i := 0.; i < (*_this).x; i++ {
		for j := 0.; j < (*_this).y; j++ {
			for _, v := range vecs {
				if float64(v.vec.x-p.y) == i && float64(v.vec.y-p.x) == j {
					fmt.Print(item)
					continue
				}
			}
...
func (_this *Map) getTileAtPos(p Entity, x, y int, sx, sy float64, vecs []Item) string {
	for _, v := range vecs {
		if float64(v.vec.x-p.y) == float64(y) && float64(v.vec.y-p.x) == float64(x) {
			return item
		}
	}
	scale := 2.
