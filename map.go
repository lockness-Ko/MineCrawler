...
func (_this *Map) getTileAtPos(p Player, x, y int) string {
	scale := 2.
	return chars[int(mapVal((*_this).noise.Noise2D(float64(math.Round((float64(y)+float64(p.y))/scale))/10, float64(math.Round((float64(x)+float64(p.x))/scale))/10)+1.1, 4, 0))]
}
